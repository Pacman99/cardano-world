...
in {
  ...
  packages-to-nix = writeShellApplication {
    name = "packages-to-nix";
    text = ''
      # go to project root directory:
      ...
      nix build .#${nixpkgs.system}.cardano.packages.generatePackagesExesMat
      ./result nix/cardano/packages/packages-exes.nix
    '';
    runtimeInputs = [cabal-project-utils.cabalProjectRegenerate];
  };
  ...
  stylish-haskell-check = nixpkgs.callPackage inputs.iohkNix.checks.stylish-haskell {
